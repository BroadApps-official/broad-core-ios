...
- Копия в iCloud только добавляется: существующая запись другого телефона не
  ...
  присоединяется к ней. Конфликт iCloud разрешается до добавления локального
  ID: другой экземпляр store не увидит промежуточный новый аккаунт. Ошибка
  добавления или перечитывания конфликта оставляет локальную запись нетронутой
  и возвращает `.failed`; повтор всё ещё может восстановить прежний аккаунт.
  Если запись устройства успел сохранить другой писатель, побеждает она.
