...
/// ``OSLogBroadLogger``, с UTC-временем в начале. ``BroadLogEvent`` принимает
/// платформенные enum, `Bool`, счётчики и заранее объявленные коды host-событий.
/// Строка собирается единственным внутренним форматтером.
///
