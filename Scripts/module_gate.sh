...
bash "$module_root/Scripts/run_contract_probes.sh"
...
xcodebuild \
    ...
    CODE_SIGNING_ALLOWED=NO \
    build
bash "$module_root/Scripts/check_privacy_manifest.sh" \
    "$derived_data/Build/Products/Release-iphoneos/$sandbox_name.app"
...
if [[ -n "$docc_warnings" ]]; then
    printf 'BroadCore DocC emitted warnings:\n%s\n' "$docc_warnings"
    exit 1
fi
