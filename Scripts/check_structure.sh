...
if ! rg -q -- 'Swinject/Swinject\.git' "$module_root/Package.swift" || \
    ! rg -q -- 'exact: "2\.10\.0"' "$module_root/Package.swift"; then
    record_failure "BroadCore must pin the verified Swinject 2.10.0 dependency."
fi

if ! bash "$module_root/Scripts/check_privacy_manifest.sh"; then
    record_failure "BroadCore privacy manifest validation failed."
fi
