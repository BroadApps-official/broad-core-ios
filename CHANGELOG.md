...
- `BroadLogHostEvent` и `BroadLogHostField` санитайзят код и поля на входе:
  всё вне `A-Za-z0-9._:-` заменяется на `-`, длина кода и полей ограничена 64
  символами, число полей — восемью. Канал остаётся типизированным, а не
  свободным текстом, поэтому в лог, который пользователь отправляет почтой,
  нельзя случайно положить URL, чек, токен или stack trace.
...
упала генерация. Открывать enum целиком нельзя — тогда в письмо уедут сырые
данные; поэтому кейс один, с фиксированной формой и очисткой значений.
