...
- `TransportErrorMessages` и `AppError.transportFailure(_:messages:diagnosticPrefix:)` —
  отказ транспорта переводится в `AppError` с видом, решением о повторе и
  стабильным кодом диагностики. Текст для пользователя остаётся за приложением.
- `DebugFlagSnapshot` и `DebugFlagStore.snapshot(of:)` — debug-флаги читаются
  один раз и дальше отвечают синхронно.

### Почему

Приложения писали свой `switch` по `URLError.Code` рядом с
`NetworkFailureClassifier`, потому что классификатор не давал `AppError`. Копии
теряли коды: потерянный DNS-запрос и выключенный роуминг попадали в «неизвестную
ошибку», а её приложение считало повторяемой.

`DebugFlagStore` асинхронный, а composition root собирает зависимости одним
синхронным проходом — и именно debug-переключатель решает, какую зависимость
собирать. Из-за этого приложения держали второй, свой синхронный стор рядом с
платформенным.
